...
class Atkinson extends BaseAlgorithm {
  constructor() {
    super('atkinson', 'Atkinson');
    this.kernel = KERNELS['atkinson'];
  }

  process(pixels, width, height, config, utils) {
    // ... La lógica es idéntica a la de FloydSteinberg
    // Se puede abstraer a una clase base "ErrorDiffusionAlgorithm"
    // para no repetir código, pero por ahora se mantiene simple.
    const { lumaLUT } = utils;
